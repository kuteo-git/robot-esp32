...
def fetch_news_from_rss(rss_url):
    """Get the news list from an RSS feed"""
    try:
        ...
        logger.bind(tag=TAG).error(f"get RSS news failed: {e}")
        return []


def fetch_news_detail(url):
    """Get the news detail page content and summarize it"""
    try:
        ...
        # Try to extract the body content (the selector here needs adjusting based on the actual site structure)
        ...
        if content_div:
            ...
            # If the specific content area isn't found, try getting all paragraphs
        ...
        logger.bind(tag=TAG).error(f"Failed to get news detail: {e}")
        return "无法获取详细内容"


def map_category(category_text):
    """Map the user's Chinese-language category input to the category key in the config file"""
    if not category_text:
        return None

    # Category mapping dict; currently supports society/international/finance news, see the config file for more types
    category_map = {
        # Society news
        "社会": "society_rss_url",
    ...
    # Return the mapped result, or the original input if there's no match
...
def get_news_from_chinanews(
    ...
    """Get the news and randomly pick one to report, or get the detail of the last news item"""
    try:
        # If detail is True, get the detail of the last news item
        if detail:
            if (
            ...
            logger.bind(tag=TAG).debug(f"Get news detail: {title}, URL={link}")
        ...
        # Otherwise, get the news list and randomly pick one
        ...
        # Map the user's input category to the config's category key
        mapped_category = map_category(category)

        # If a category was provided, try to get the corresponding URL from config
        ...
        # Save the current news link on the connection object so detail can be looked up later
