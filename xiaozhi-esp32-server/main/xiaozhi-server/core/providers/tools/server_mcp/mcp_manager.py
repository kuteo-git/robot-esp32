...
class ServerMCPManager:
    ...
    def __init__(self, conn) -> None:
        ...
        if not os.path.exists(self.config_path):
            ...
            logger.bind(tag=TAG).warning(
                f"Check MCP config file: data/.mcp_server_settings.json"
            )
    ...
    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        """执行工具调用，失败时会尝试重新连接"""
        logger.bind(tag=TAG).info(f"Execute server MCP tool {tool_name} , params: {arguments}")
    ...
    async def cleanup_all(self) -> None:
        ...
        for name, client in list(self.clients.items()):
            try:
                ...
                logger.bind(tag=TAG).error(f"Close server MCP Client {name} error during: {e}")
        self.clients.clear()
