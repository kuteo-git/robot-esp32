...
class ToolManager:
    ...
    def register_executor(self, tool_type: ToolType, executor: ToolExecutor):
        ...
        self.logger.debug(f"Register tool executor: {tool_type.value}")
    ...
    def get_all_tools(self) -> Dict[str, ToolDefinition]:
        ...
        for tool_type, executor in self.executors.items():
            try:
                ...
                self.logger.error(f"get {tool_type.value} error on tool: {e}")
    ...
    async def execute_tool(
        ...
        try:
            ...
            if not executor:
                return ActionResponse(
                    action=Action.ERROR,
                    response=f"loại công cụ {tool_type.value} executor chưa đăng ký",
                )

            # 执行工具
            self.logger.info(f"Execute tool: {tool_name} , params: {arguments}")
            result = await executor.execute(self.conn, tool_name, arguments)
            self.logger.debug(f"Tool execution result: {result}")
            return result

        except Exception as e:
            self.logger.error(f"Execute tool {tool_name} error during: {e}")
    ...
    def get_tool_statistics(self) -> Dict[str, int]:
        ...
        for tool_type, executor in self.executors.items():
            try:
                ...
                self.logger.error(f"get {tool_type.value} Error getting tool stats: {e}")
