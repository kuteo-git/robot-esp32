    ...
    async def execute(
        ...
        if not self.has_tool(tool_name):
            return ActionResponse(
                action=Action.NOTFOUND, response=f"IoT công cụ {tool_name} không tồn tại"
            )
        ...
        try:
            ...
            return ActionResponse(action=Action.ERROR, response="không phân tích được IoT tên công cụ")
