...
async def send_mcp_message(conn: "ConnectionHandler", payload: dict):
    ...
    try:
        ...
        logger.bind(tag=TAG).debug(f"Sent successfully MCP message: {message}")
    ...
    if "result" in payload:
        ...
        if msg_id in mcp_client.call_results:
            logger.bind(tag=TAG).debug(
                f"Tool call response, ID: {msg_id}, result: {result}"
            )
            await mcp_client.resolve_call_result(msg_id, result)
            return

        if msg_id == 1:  # mcpInitializeID
            logger.bind(tag=TAG).debug("Received MCP Init response")
            ...
            logger.bind(tag=TAG).debug("Init complete, starting request MCP Tool list")
            ...
            if isinstance(result, dict) and "tools" in result:
                ...
                logger.bind(tag=TAG).info(
                    f"Client device supported tools: {len(tools_data)}"
                )
...
async def send_mcp_initialize_message(conn: "ConnectionHandler"):
    ...
    logger.bind(tag=TAG).debug("send MCP Init message")
...
async def send_mcp_tools_list_continue_request(conn: "ConnectionHandler", cursor: str):
    ...
    logger.bind(tag=TAG).info(f"send with cursor   MCP Tool list request: {cursor}")
    await send_mcp_message(conn, payload)
...
async def call_mcp_tool(
                ...
                try:
                    ...
                    try:
                        ...
                        if len(json_objects) > 1:
                            ...
                            raise ValueError(f"Args JSON parse failed: {args}")
                    except Exception as e:
                        logger.bind(tag=TAG).error(
                            f"Args JSON parse failed: {str(e)}, raw args: {args}"
                        )
                        raise ValueError(f"Args JSON parse failed: {str(e)}")
        elif isinstance(args, dict):
            arguments = args
        else:
            raise ValueError(f"Args type error, expected str/dict, got: {type(args)}")

        # 确保参数是字典类型
        if not isinstance(arguments, dict):
            raise ValueError(f"Args must be dict, got: {type(arguments)}")

    except Exception as e:
        if not isinstance(e, ValueError):
            raise ValueError(f"Args processing failed: {str(e)}")
        raise e
    ...
    logger.bind(tag=TAG).info(f"Send client mcp Tool call request: {actual_name} , params: {args}")
    await send_mcp_message(conn, payload)
    ...
    try:
        ...
        logger.bind(tag=TAG).info(
            f"Client MCP tool call {actual_name} OK, raw result: {raw_result}"
        )

        if isinstance(raw_result, dict):
            if raw_result.get("isError") is True:
                error_msg = raw_result.get(
                    "error", "Tool call returned error without details"
                )
