...
class UnifiedToolHandler:
    ...
    async def _initialize(self):
        ...
        try:
            ...
            self.logger.debug("Unified tool handler init complete")
        ...
        try:
            ...
            if (
                ...
                if mcp_endpoint_client:
                    ...
                    self.logger.info("MCP Endpoint init succeeded")
                else:
                    self.logger.warning("MCP Endpoint init failed")

        except Exception as e:
            self.logger.error(f"Initialize MCP Endpoint failed: {e}")
    ...
    def _initialize_home_assistant(self):
        ...
        try:
            ...
            self.logger.error(f"Initialize Home Assistant failed: {e}")
    ...
    async def handle_llm_function_call(
        ...
        try:
            ...
            if isinstance(arguments, str):
                try:
                    ...
                    return ActionResponse(
                        action=Action.ERROR,
                        response="Không phân tích được tham số hàm",
                    )

            self.logger.debug(f"Call function: {function_name}, params: {arguments}")
            ...
            self.logger.error(f"Process function call error: {e}")
            return ActionResponse(action=Action.ERROR, response=str(e))

    def _combine_responses(self, responses: List[ActionResponse]) -> ActionResponse:
        """合并多个函数调用的响应"""
        if not responses:
            return ActionResponse(action=Action.NONE, response="không phản hồi")
