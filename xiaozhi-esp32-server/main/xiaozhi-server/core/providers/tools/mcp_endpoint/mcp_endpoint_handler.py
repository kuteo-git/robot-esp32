...
async def connect_mcp_endpoint(mcp_endpoint_url: str, conn=None) -> MCPEndpointClient:
    ...
    try:
        ...
        logger.bind(tag=TAG).info("MCP Endpoint connected")
        return mcp_client

    except Exception as e:
        logger.bind(tag=TAG).error(f"connection MCP Endpoint failed: {e}")
        return None
    ...
    try:
        ...
        if "result" in payload:
            ...
            if msg_id == 1:  # mcpInitializeID
                logger.bind(tag=TAG).debug("Received MCP Endpoint init response")
        ...
        logger.bind(tag=TAG).error(f"Process MCP Error on endpoint message: {e}")
        import traceback
...
async def send_mcp_endpoint_initialize(mcp_client: MCPEndpointClient):
    ...
    logger.bind(tag=TAG).info("send MCP Endpoint init message")
...
async def send_mcp_endpoint_notification(mcp_client: MCPEndpointClient, method: str):
    ...
    message = json.dumps(payload)
    logger.bind(tag=TAG).debug(f"send MCP Endpoint notification: {method}")
    await mcp_client.send_message(message)
...
async def send_mcp_endpoint_tools_list(mcp_client: MCPEndpointClient):
    ...
    message = json.dumps(payload)
    logger.bind(tag=TAG).debug("send MCP Endpoint tool list request")
    await mcp_client.send_message(message)
...
async def send_mcp_endpoint_tools_list_continue(
    ...
    message = json.dumps(payload)
    logger.bind(tag=TAG).info(f"send with cursor   MCP Endpoint tool list request: {cursor}")
    await mcp_client.send_message(message)
