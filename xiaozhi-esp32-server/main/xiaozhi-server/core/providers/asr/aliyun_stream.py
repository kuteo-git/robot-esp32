...
class ASRProvider(ASRProviderBase):
    ...
    async def _start_recognition(self, conn: "ConnectionHandler"):
        ...
        logger.bind(tag=TAG).debug("Start request sent, waiting for server...")
        ...
        try:
            while not conn.stop_event.is_set():
                ...
                try:
                    ...
                    if status != 20000000:
                        if status == 40010004:
                            logger.bind(tag=TAG).warning(f"Close connection only after server response completes, status code: {status}")
                            break
                    ...
                    logger.bind(tag=TAG).error("Timeout receiving result")
                    break
    ...
    async def _cleanup(self):
        """清理资源（关闭连接）"""
        logger.bind(tag=TAG).debug(f"start ASR Session cleanup | Current status: processing={self.is_processing}, server_ready={self.server_ready}")
        ...
        if self.asr_ws:
            try:
                ...
                logger.bind(tag=TAG).error(f"close WebSocket Connection failed: {e}")
            finally:
                self.asr_ws = None
    ...
    async def close(self):
        ...
        if hasattr(self, 'decoder') and self.decoder is not None:
            try:
                ...
                logger.bind(tag=TAG).debug(f"release Aliyun decoder Error on resource: {e}")
