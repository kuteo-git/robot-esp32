...
class ASRProvider(ASRProviderBase):
    ...
    async def _start_recognition(self, conn: "ConnectionHandler"):
        ...
        try:
            ...
            logger.bind(tag=TAG).debug("sent run-task command, waiting for server...")
    ...
    async def _send_finish_task(self):
        ...
        if self.asr_ws and self.task_id:
            try:
                ...
                logger.bind(tag=TAG).error(f"send finish-task command failed: {e}")
    ...
    async def close(self):
        ...
        if hasattr(self, 'decoder') and self.decoder is not None:
            try:
                ...
                logger.bind(tag=TAG).debug(f"release Aliyun BL decoder Error on resource: {e}")