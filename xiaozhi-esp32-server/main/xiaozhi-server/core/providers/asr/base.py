...
class ASRProviderBase(ABC):
        ...
        try:
            ...
            if isinstance(raw_text, dict):
                ...
                if raw_text.get("language"):
                    logger.bind(tag=TAG).info(f"Recognition language: {raw_text['language']}")
                if raw_text.get("emotion"):
                    logger.bind(tag=TAG).info(f"Recognize emotion: {raw_text['emotion']}")
    ...
    def _pcm_to_wav(self, pcm_data: bytes) -> bytes:
        """将PCM数据转换为WAV格式"""
        if len(pcm_data) == 0:
            logger.bind(tag=TAG).warning("PCM Data empty, cannot convert WAV")
            return b""
    ...
    def build_temp_file(self, pcm_bytes: bytes) -> Optional[str]:
        try:
            ...
            logger.bind(tag=TAG).error(f"Temp audio file generation failed: {e}")
            return None
        ...
        try:
            ...
            if decoder is not None:
                try:
                    ...
                    logger.bind(tag=TAG).debug(f"release decoder Error on resource: {e}")
