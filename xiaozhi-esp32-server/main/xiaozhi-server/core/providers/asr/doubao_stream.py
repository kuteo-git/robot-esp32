    ...
    async def receive_audio(self, conn: "ConnectionHandler", audio, audio_have_voice):
        ...
        if audio_have_voice and self.asr_ws is None and not self.is_processing:
            try:
                ...
                logger.bind(tag=TAG).info(f"Connecting ASR service, headers: {headers}")
                ...
                try:
                    ...
                    logger.bind(tag=TAG).info(f"Send init request: {request_params}")
                    await self.asr_ws.send(full_client_request)

                    # 等待初始化响应
                    init_res = await self.asr_ws.recv()
                    result = self.parse_response(init_res)
                    logger.bind(tag=TAG).info(f"Received init response: {result}")
                    ...
                    logger.bind(tag=TAG).error(f"Failed to send init request: {str(e)}")
                ...
                logger.bind(tag=TAG).error(f"establish ASR Connection failed: {str(e)}")
                if hasattr(e, "__cause__") and e.__cause__:
                    logger.bind(tag=TAG).error(f"Error reason: {str(e.__cause__)}")
                if self.asr_ws:
        ...
        try:
            while self.asr_ws and not conn.stop_event.is_set():
                ...
                try:
                        ...
                        if "result" in payload:
                            ...
                            # 专门处理没有文本的识别结果（手动模式下可能已经识别完成但是没松按键）
                                ...
                                if conn.client_listen_mode == "manual" and conn.client_voice_stop and len(audio_data) > 15:
                                    logger.bind(tag=TAG).debug("Stop signal at message end, triggering processing")
                                    await self.handle_voice_stop(conn, audio_data)
                                    break
                    ...
                    logger.bind(tag=TAG).error(f"Process ASR Error on result: {str(e)}")
                    if hasattr(e, "__cause__") and e.__cause__:
                        logger.bind(tag=TAG).error(f"Error reason: {str(e.__cause__)}")
                    self.is_processing = False
                    break
            ...
            logger.bind(tag=TAG).error(f"ASR Error in result forwarding task: {str(e)}")
            if hasattr(e, "__cause__") and e.__cause__:
                logger.bind(tag=TAG).error(f"Error reason: {str(e.__cause__)}")
        finally:
            if self.asr_ws:
    ...
    def parse_response(self, res: bytes) -> dict:
        try:
            ...
            try:
                ...
                logger.bind(tag=TAG).debug(f"Parsed successfully JSON response: {result}")
    ...
    async def close(self):
        ...
        if hasattr(self, "decoder") and self.decoder is not None:
            try:
                ...
                logger.bind(tag=TAG).debug(f"release Doubao decoder Error on resource: {e}")
