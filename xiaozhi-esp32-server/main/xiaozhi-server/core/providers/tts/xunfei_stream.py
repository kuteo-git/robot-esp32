            ...
            try:
                ...
                if message.sentence_type == SentenceType.FIRST:
                    ...
                    try:
                        ...
                        logger.bind(tag=TAG).debug("Starting up TTS session...")
                        ...
                        logger.bind(tag=TAG).error(f"start TTS Session failed: {str(e)}")
                        continue
                ...
                if ContentType.TEXT == message.content_type:
                    if message.content_detail:
                        try:
                            ...
                            logger.bind(tag=TAG).error(f"send TTS text failed: {str(e)}")
                ...
                if message.sentence_type == SentenceType.LAST:
                    try:
                        logger.bind(tag=TAG).debug("Start ending TTS session...")
                        asyncio.run_coroutine_threadsafe(
                            self.finish_session(self.conn.sentence_id),
                            loop=self.conn.loop,
                        )
                    except Exception as e:
                        logger.bind(tag=TAG).error(f"end TTS Session failed: {str(e)}")
                        continue
    ...
    async def start_session(self, session_id):
        ...
        try:
            ...
            if self._monitor_task is None or self._monitor_task.done():
                logger.bind(tag=TAG).debug("Start listen task...")
            ...
            if self.ws:
                ...
                if self._monitor_task:
                    try:
                        ...
                        logger.bind(tag=TAG).error(f"Error waiting for listen task to finish: {str(e)}")
                    finally:
                        self._monitor_task = None
        except Exception as e:
    ...
    async def close(self):
        ...
        if self._monitor_task:
            try:
                ...
                logger.bind(tag=TAG).warning(f"Error cancelling listen task on close: {e}")
            self._monitor_task = None
        ...
        try:
            while not self.conn.stop_event.is_set():
                try:
                    ...
                    # 检查客户端是否中止
                    if self.conn.client_abort:
                        logger.bind(tag=TAG).info("Received interrupt, terminating listen TTS response")
                        break
                        ...
                        if code == 0:
                            ...
                            if audio_payload:
                                ...
                                if status == 0:
                                    ...
                                    logger.bind(tag=TAG).debug("Received end-state audio data, TTS Synthesis complete")
                                    self.activate_session = False
