...
THINKING_DISABLED_DOMAINS = {
    ...
    "moonshot.ai": {"thinking": {"type": "disabled"}},  # Kimi K2.5 quốc tế: tắt thinking -> nhanh ~2s (cần temperature=0.6)
    ...
    def _apply_thinking_disabled(self, request_params: dict):
        ...
        for disabled_domain, params in THINKING_DISABLED_DOMAINS.items():
            if disabled_domain in domain:
                ...
                logger.bind(tag=TAG).info(f"for domain {domain} Disable thinking mode, params: {params}")
                break
