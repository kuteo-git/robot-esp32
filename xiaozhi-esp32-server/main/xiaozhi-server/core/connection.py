...
class ConnectionHandler:
    ...
    async def _process_mqtt_audio_message(self, message):
        ...
        try:
            ...
            self.logger.bind(tag=TAG).error(f"parse WebSocket Audio packet failed: {e}")
    ...
    def _initialize_components(self):
        try:
            ...
            if self.config.get("prompt") is not None:
                ...
                self.logger.bind(tag=TAG).info(
                    f"Fast init component: prompt OK {prompt[:50]}..."
                )
    ...
    def _init_prompt_enhancement(self, base_prompt: str = None):
        ...
        # FIX: enhanced_prompt vừa ghi đè conn.prompt nên danh sách thiết bị HA
        # (đã append lúc init intent) bị mất -> append LẠI sau khi enhance để LLM thấy entity_id thật.
        if self.intent_type == "function_call":
            try:
                from plugins_func.functions.hass_init import append_devices_to_prompt
                append_devices_to_prompt(self)
            except Exception as e:
                self.logger.bind(tag=TAG).warning(f"append HA devices sau enhance lỗi: {e}")
    ...
    def _inject_tool_call_fewshot(self):
        ...
        if "handle_exit_intent" in tool_names:
            ...
            self.dialogue.put(Message(
                ...
                tool_calls=[{
                    "id": tc_id,
                    "function": {"arguments": '{"say_goodbye": "Tạm biệt, lần sau tám tiếp nha~"}', "name": "handle_exit_intent"},
                    "type": "function", "index": 0,
                }],
                is_temporary=True,
            ))
        ...
        self.logger.bind(tag=TAG).debug("Tool call injected few-shot example")
        ...
        try:
            ...
            if voiceprint_config:
                ...
                if voiceprint_provider is not None and voiceprint_provider.enabled:
                    ...
                    self.logger.bind(tag=TAG).info("Voiceprint recognition dynamically enabled on connect")
                else:
    ...
    async def _background_initialize(self):
        ...
        try:
            ...
            self.logger.bind(tag=TAG).error(f"Background init failed: {e}")
    ...
    async def _initialize_private_config_async(self):
        ...
        try:
            ...
            self.need_bind = True
            self.logger.bind(tag=TAG).error(f"Failed to async fetch differentiated config: {e}")
            private_config = {}
            ...
            if not bHasError and len(tool_calls_list) > 0:
                ...
                for future, tool_call_data, tool_input in futures_with_data:
                    try:
                        ...
                        # 超时时返回错误响应，避免整个流程卡死
                        tool_results.append((
                            ActionResponse(action=Action.ERROR, result="Ui, mạng đang trục trặc chút, lát thử lại nha!"),
                            tool_call_data
                        ))
                        # 上报工具调用错误
    ...
    async def close(self, ws=None):
        ...
        try:
            ...
            try:
                ...
                self.logger.bind(tag=TAG).error(f"close WebSocket Error during connection: {ws_error}")
