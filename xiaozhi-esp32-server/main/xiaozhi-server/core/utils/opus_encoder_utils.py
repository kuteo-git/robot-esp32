...
class OpusEncoderUtils:
    ...
    def __init__(self, sample_rate: int, channels: int, frame_size_ms: int):
        ...
        # Bitrate: 24k đủ cho GIỌNG nhưng NÁT NHẠC (48kHz mono cần nhiều bit hơn) -> 96k (env OPUS_BITRATE).
        # Encoder dùng chung TTS + nhạc; 96k cho nhạc hay, giọng cũng tốt hơn, băng thông wifi thừa sức.
        self.bitrate = int(os.environ.get("OPUS_BITRATE", "96000"))  # bps
        ...
        try:
            ...
            self.encoder.signal = constants.AUTO  # AUTO: Opus tự nhận giọng/nhạc (encoder dùng chung TTS+nhạc; VOICE làm dở nhạc)
        except Exception as e:
            logging.error(f"Initialize Opus Encoder failed: {e}")
    ...
    def _validate_pcm_data(self, pcm_shorts: np.ndarray) -> None:
        ...
        if np.any((pcm_shorts < -32768) | (pcm_shorts > 32767)):
            ...
            logging.warning(f"found invalid PCM sample: {invalid_samples[:5]}...")
            # 在实际应用中可以选择裁剪而不是抛出异常
