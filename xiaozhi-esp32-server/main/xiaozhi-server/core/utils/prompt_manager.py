...
class PromptManager:
    ...
    def _load_base_template(self):
        ...
        try:
            ...
            if os.path.exists(template_path):
                ...
                self.logger.bind(tag=TAG).warning(f"not found {template_path} file")
    ...
    def get_quick_prompt(self, user_prompt: str, device_id: str = None) -> str:
        ...
        if cached_device_prompt is not None:
            self.logger.bind(tag=TAG).debug(f"using device {device_id} cached prompt")
            return cached_device_prompt
        else:
            self.logger.bind(tag=TAG).debug(
                f"Device {device_id} no cached prompt, using provided prompt"
            )
        ...
        if device_id:
            ...
            self.logger.bind(tag=TAG).debug(f"device {device_id} 's prompt cached")
    ...
    def build_enhanced_prompt(
        ...
        try:
            ...
            self.logger.bind(tag=TAG).info(
                f"Enhanced prompt built, length: {len(enhanced_prompt)}"
            )
