    ...
    try:
        ...
        if decoder is not None:
            try:
                ...
                conn.logger.bind(tag=TAG).debug(f"release decoder Error on resource: {e}")
...
def enqueue_tts_report(conn: "ConnectionHandler", text, opus_data):
    ...
    try:
        ...
        conn.logger.bind(tag=TAG).error(f"join TTS Report enqueue failed: {text}, {e}")
...
def enqueue_asr_report(conn: "ConnectionHandler", text, opus_data):
    ...
    try:
        ...
        conn.logger.bind(tag=TAG).debug(f"join ASR Report enqueue failed: {text}, {e}")
