    ...
    if conn.last_activity_time > 0.0:
        ...
        if (
            ...
            if end_prompt and end_prompt.get("enable", True) is False:
                conn.logger.bind(tag=TAG).info("End conversation, no need to send closing prompt")
                await conn.close()
                return
...
async def max_out_size(conn: "ConnectionHandler"):
    # 播放超出最大输出字数的提示
    conn.client_abort = False
    text = "Xin lỗi, giờ tao đang bận chút việc, mai giờ này mình tám tiếp nha, hẹn rồi đó! Mai không gặp không về, bái bai!"
...
async def check_bind_device(conn: "ConnectionHandler"):
    if conn.bind_code:
        ...
        if len(conn.bind_code) != 6:
            ...
            text = "Mã ghép sai định dạng, kiểm tra lại cấu hình nha."
            await send_stt_message(conn, text)
            return
