        ...
        if msg_json["state"] == "start":
            ...
            if "text" in msg_json:
                ...
                if is_wakeup_words and not enable_greeting:
                    # Wake word + tắt lời chào: KHÔNG gọi LLM (tránh LLM bịa từ mỗi chữ "Alexa").
                    # Tuỳ chọn: phát 1 tiếng "ding" ngắn báo đã thức (wakeup_awake_sound), rồi sẵn sàng nghe lệnh.
                    await send_stt_message(conn, original_text)
                    if conn.config.get("wakeup_awake_sound", False):
                        try:
                            awake_voice = conn.config.get(
                                "wakeup_awake_voice", "config/assets/tts_notify.mp3"
                            )
                            await send_tts_message(conn, "start")
                            audios = await audio_to_data(awake_voice, is_opus=True)
                            await sendAudio(conn, audios)
                        except Exception as e:
                            conn.logger.bind(tag=TAG).warning(f"Phát tiếng awake lỗi: {e}")
