...
class OTAHandler(BaseHandler):
    ...
    def generate_password_signature(self, content: str, secret_key: str) -> str:
        ...
        try:
            ...
            self.logger.bind(tag=TAG).error(f"generate MQTT Password signing failed: {e}")
            return ""
    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            if device_id:
                ...
                raise Exception("OTA request device ID empty")

            client_id = request.headers.get("client-id", "")
            if client_id:
                self.logger.bind(tag=TAG).info(f"OTA request ClientID: {client_id}")
            else:
                raise Exception("OTA request ClientID empty")
            ...
            if mqtt_gateway_endpoint:  # 如果配置了非空字符串
                ...
                if signature_key:
                    ...
                    self.logger.bind(tag=TAG).warning("missing MQTT Signing key, password left empty")
                ...
                self.logger.bind(tag=TAG).info(f"for device {device_id} dispatch MQTT Gateway config")
            ...
            try:
                ...
                self.logger.bind(tag=TAG).info(
                    f"Firmware lookup model {device_model}: {len(candidates)} candidates"
                )
                ...
                if chosen_url:
                    ...
                    self.logger.bind(tag=TAG).info(
                        f"Sending firmware {chosen_version} to device {device_id} [if URL prefix wrong, check server.vision_explain]-> {chosen_url} "
                    )
                else:
                    self.logger.bind(tag=TAG).info(
                        f"Device {device_id} firmware up to date: {device_version}"
                    )
    ...
    async def handle_get(self, request):
        ...
        try:
            ...
            message = f"OTA interface running normally, websocket address sent to device: {websocket_url}"
            response = web.Response(text=message, content_type="text/plain")
        except Exception as e:
            self.logger.bind(tag=TAG).error(f"OTA GET Request exception: {e}")
            response = web.Response(text="OTA Giao diện lỗi", content_type="text/plain")
        finally:
            self._add_cors_headers(response)
            return response
