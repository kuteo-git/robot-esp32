...
| **news bulletin** | 8014 | `cd services && conda run -n xiaozhi python news_server.py` | `NEWS_LLM_BASE_URL`/`NEWS_LLM_MODEL`/`NEWS_LLM_API_KEY` (any OpenAI-compatible endpoint) + `ffmpeg`; optional `NEWS_START_WAV`/`NEWS_END_WAV` stings. Produces the full spoken bulletin (`get_news_bulletin`). |
