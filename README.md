...
| **news bulletin** | 8014 | `cd services && conda run -n xiaozhi python news_server.py` | `NEWS_LLM_BASE_URL`/`NEWS_LLM_MODEL`/`NEWS_LLM_API_KEY` (any OpenAI-compatible endpoint) + `ffmpeg`; optional `NEWS_START_WAV`/`NEWS_END_WAV` stings and `NEWS_GAP_SEC` (0.9s of silence between stories, 0 disables). Produces the full spoken bulletin (`get_news_bulletin`). Saying exactly "đọc bản tin" / "nghe bản tin" (which is also what the daily alarm and the panel's test button send) runs the tool **directly, with no LLM turn in front of it** — see `intentHandler.check_direct_news`. Any longer phrasing still goes through the LLM as normal. |
